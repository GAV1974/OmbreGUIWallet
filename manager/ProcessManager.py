...
## Copyright (c) 2017, The Sumokoin Project (www.sumokoin.org)
'''
Process managers for ombred, ombre-wallet-cli and ombre-wallet-rpc
'''
...
class SumokoindManager(ProcessManager):
    def __init__(self, resources_path, log_level=0, block_sync_size=10):
        proc_args = u'%s/bin/ombred --log-level %d --block-sync-size %d' % (resources_path, log_level, block_sync_size)
        ProcessManager.__init__(self, proc_args, "ombred")
        self.synced = Event()
        self.stopped = Event()
